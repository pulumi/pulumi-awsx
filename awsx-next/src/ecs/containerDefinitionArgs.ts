...
export interface ContainerDefinitionArgs {
   ...
   * user for better security. You can specify the ``user`` using the following formats. If
   * specifying a UID or GID, you must specify it as a positive integer. + ``user`` + ``user:group``
...
export interface LogConfiguration {
   ...
   * + awslogs-group Required: Yes Make sure to specify a log group that the awslogs log driver
   * sends its log streams to. + awslogs-stream-prefix Required: Yes, when using Fargate.Optional
   * when using EC2. Use the awslogs-stream-prefix option to associate a log stream with the
   ...
   * recommend that you specify a prefix with this option. For Amazon ECS services, you can use the
   * service name as the prefix. Doing so, you can trace log streams to the service that the
   * container belongs to, the name of the container that sent them, and the ID of the task that the
   * container belongs to. You must specify a stream-prefix for your logs to have your logs appear
   * in the Log pane when using the Amazon ECS console. + awslogs-datetime-format Required: No This
   * option defines a multiline start pattern in Python strftime format. A log message consists of a
   * line that matches the pattern and any following lines that don’t match the pattern. The matched
   * line is the delimiter between log messages. One example of a use case for using this format is
   * for parsing output such as a stack dump, which might otherwise be logged in multiple entries.
   * The correct pattern allows it to be captured in a single entry. For more information, see
   ...
   * awslogs-multiline-pattern Required: No This option defines a multiline start pattern that uses
   * a regular expression. A log message consists of a line that matches the pattern and any
   * following lines that don’t match the pattern. The matched line is the delimiter between log
   * messages. For more information, see awslogs-multiline-pattern. This option is ignored if
   * awslogs-datetime-format is also configured. You cannot configure both the
   * awslogs-datetime-format and awslogs-multiline-pattern options. Multiline logging performs
   * regular expression parsing and matching of all log messages. This might have a negative impact
   * on logging performance. The following options apply to all supported log drivers. + mode
   * Required: No Valid values: non-blocking | blocking This option defines the delivery mode of log
   * messages from the container to the log driver specified using logDriver. The delivery mode you
   * choose affects application availability when the flow of logs from container is interrupted. If
   * you use the blocking mode and the flow of logs is interrupted, calls from container code to
   * write to the stdout and stderr streams will block. The logging thread of the application will
   * block as a result. This may cause the application to become unresponsive and lead to container
   * healthcheck failure. If you use the non-blocking mode, the container's logs are instead stored
   * in an in-memory intermediate buffer configured with the max-buffer-size option. This prevents
   * the application from becoming unresponsive when logs cannot be sent. We recommend using this
   * mode if you want to ensure service availability and are okay with some log loss. For more
   * information, see Preventing log loss with non-blocking mode in the awslogs container log
   * driver. You can set a default mode for all containers in a specific Region by using the
   ...
   * the account setting, see Default log driver mode in the Amazon Elastic Container Service
   * Developer Guide. On June 25, 2025, Amazon ECS changed the default log driver mode from blocking
   * to non-blocking to prioritize task availability over logging. To continue using the blocking
   * mode after this change, do one of the following: Set the mode option in your container
   * definition's logConfiguration as blocking. Set the defaultLogDriverMode account setting to
   * blocking. + max-buffer-size Required: No Default value: 10m When non-blocking mode is used, the
   * max-buffer-size log option controls the size of the buffer that's used for intermediate message
   * storage. Make sure to specify an adequate buffer size based on your application. When the
   * buffer fills up, further logs cannot be stored. Logs that cannot be stored are lost. To route
   * logs using the `splunk` log router, you need to specify a `splunk-token` and a `splunk-url`.
   ...
   * result in memory running out for the buffer inside of Docker. Other options you can specify
   * when using `awsfirelens` to route logs depend on the destination. When you export logs to
   * Amazon Data Firehose, you can specify the AWS Region with `region` and a name for the log
   * stream with `delivery_stream`. When you export logs to Amazon Kinesis Data Streams, you can
   * specify an AWS Region with `region` and a data stream name with `stream`. When you export logs
   * to Amazon OpenSearch Service, you can specify options like `Name`, `Host` (OpenSearch Service
   * endpoint without protocol), `Port`, `Index`, `Type`, `Aws_auth`, `Aws_region`,
   * `Suppress_Type_Name`, and `tls`. For more information, see [Under the hood: FireLens for Amazon
   * ECS
...
export interface PortMapping {
   ...
   * `containerPortRange`, leave this field empty and the value of the `hostPort` is set as follows:
