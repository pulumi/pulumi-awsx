...
A Cluster defines the infrastructure to run Services and Tasks in.  If a Cluster is not specified when creating Services or running Tasks, then a default one will be created that is confired to use [The Default VPC](https://github.com/pulumi/pulumi-aws-infra/tree/master/nodejs/aws-infra/ec2#the-default-vpc) for your region.  Creating a Cluster that uses a different Vpc can be simply done by:
...
However, `image` is far more flexible than that.  Beyond just accepting a string a [ContainerImageProvider](https://github.com/pulumi/pulumi-aws-infra/blob/8d651854ba3821644eabff66c0f6fe6d85e61160/nodejs/aws-infra/ecs/container.ts#L188) can also be provided.  Instances of this interface can be used to dynamically compute and pass in an ECR repository path.  Pulumi provides several convenient ways to do this.  
