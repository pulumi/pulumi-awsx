...
1. Read `AGENTS.md` before editing files.
1. Prefer `make test_provider` for fast, local validation.
1. If schema-generation inputs changed (for example `provider/pkg/schemagen/**`), run `make generate` and commit generated diffs.
1. Use `make test` only for integration-level behavior changes or when explicitly requested, since it creates real AWS resources.
1. Include command evidence in PRs (commands run and pass/fail outcomes).
