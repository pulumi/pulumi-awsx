  ...
  for (let i = 0; i < azNames.length; i++) {
    subnetInputs
      ...
      .forEach((privateSubnet) => {
        ...
        privateSubnets.push({
          ...
          type: "Private",
          subnetName: `${vpcName}-${privateSubnet.name ?? "private"}-${i + 1}`,
        });
      });
    ...
    subnetInputs
      ...
      .forEach((publicSubnet) => {
        ...
        const splitBase = privateSubnets.length > 0 ? cidrSubnetV4(baseCidr, 0, 1) : azBases[i];
        ...
        publicSubnets.push({
          ...
          type: "Public",
          subnetName: `${vpcName}-${publicSubnet.name ?? "public"}-${i + 1}`,
        });
      });
    ...
    subnetInputs
      ...
      .forEach((isolatedSubnet) => {
        ...
        isolatedSubnets.push({
          ...
          type: "Isolated",
          subnetName: `${vpcName}-${isolatedSubnet.name ?? "isolated"}-${i + 1}`,
        });
      });
  }
